...
function checkMarkdownFiles(): boolean {
  ...
  console.log(`${colorize('✅ 最佳长度:', 'green')} ${MIN_LENGTH}-${MAX_LENGTH} 字符`);
  ...
  for (const filePath of allFiles) {
    try {
      ...
      if (severity === 'good') {
        ...
        console.log(`   ${colorize('"' + description.substring(0, 100) + (description.length > 100 ? '...' : '') + '"', 'blue')}`);
  ...
  console.log(`${colorize('需要优化:', 'yellow')} ${issues.filter(i => i.severity === 'warning').length}`);
  console.log(`${colorize('错误:', 'red')} ${issues.filter(i => i.severity === 'error').length}`);
  console.log(`${colorize('通过率:', passedFiles >= totalFiles * 0.8 ? 'green' : 'yellow')} ${((passedFiles / totalFiles) * 100).toFixed(1)}%`);
  ...
  if (issues.length > 0) {
    ...
    issues.forEach(issue => {
      ...
      if (issue.description && issue.length !== undefined) {
        console.log(`  建议: ${issue.length < MIN_LENGTH ? '增加更多描述性内容' : '精简描述内容'}`);
      }
      console.log('');
    });
  }
  ...
  if (fs.existsSync(configPath)) {
    ...
    if (descMatch) {
      ...
      if (desc.length < MIN_LENGTH) {
        console.log(colorize('⚠️  默认描述太短，建议增加到 150-160 字符', 'yellow'));
      } else if (desc.length > MAX_LENGTH) {
        console.log(colorize('⚠️  默认描述太长，建议控制在 150-160 字符', 'yellow'));
      } else {
...
function main(): void {
  ...
  if (allPassed) {
    console.log(colorize('所有页面的 Meta descriptions 都符合 SEO 最佳实践！', 'green'));
    process.exit(0);
  } else {
