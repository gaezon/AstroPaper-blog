...
test.describe("Sitemap hreflang 验证", () => {
    ...
    test.beforeAll(() => {
        ...
        // 本地环境检查文件是否在合理时间内生成
        const skipFreshnessCheck = process.env.CI || process.env.SKIP_SITEMAP_FRESHNESS_CHECK;

        if (!skipFreshnessCheck) {
            const stats = fs.statSync(sitemapPath);
            const fileAge = Date.now() - stats.mtimeMs;
            // 本地环境使用 30 分钟阈值，避免慢速构建导致误报
            const thresholdMinutes = parseInt(process.env.SITEMAP_FRESHNESS_MINUTES || "30", 10);
